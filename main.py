...
template = """
    ...
    - Convertir el borrador de texto a un tono específico
    - Convertir el borrador de texto a un dialecto específico

    He aquí algunos ejemplos diferentes Tonos:
    - Formal: ¡Saludos! OpenAI ha anunciado que Sam Altman se reincorpora a la empresa como Consejero Delegado. Tras un periodo de cinco días de conversaciones, discusiones y deliberaciones, se ha tomado la decisión de traer de vuelta a Altman, que había sido despedido previamente. Estamos encantados de dar de nuevo la bienvenida a Sam a OpenAI.
...
with col1:
    st.markdown("Vuelve a escribir el texto en diferentes estilos.")
...
def get_draft():
    draft_text = st.text_area(label="Texto", label_visibility='collapsed', placeholder="Su texto...", key="draft_input")
    return draft_text
...
# Prompt template opciones de ajuste
col1, col2 = st.columns(2)
with col1:
    option_tone = st.selectbox(
        '¿Qué tono le gustaría que tuviera su redacción?',
        ('Formal', 'Informal'))
